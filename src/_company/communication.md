...
1. If a meeting needs an additional document for internal documentation, create a new document in the appropriate folder in the `External Meetings` folder in the shared `Meltano Team` Google Drive and link to it in the public notes doc. Preface the link with "Internal Meltano Ref: `link`". If a relevant private GitLab issue exists as well, link to it inline with the Google Doc link. 

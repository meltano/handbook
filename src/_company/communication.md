...
1. If a meeting needs an additional document for internal documentation, create an issue in an appropriate [issue tracker](/company/using-gitlab#issue-trackers), mark it as confidential if the project is public, link a new Google Doc to the issue, share the Google Doc with the appropriate people, and link the issue to the calendar invite. Indicate in the invite description that the link to the GitLab issue is "Internal Only".
